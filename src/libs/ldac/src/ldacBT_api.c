...
/* Set target bitrate in kbps. LDAC represents rate as an integer transport
 * frame length, so values that are not exact are rounded to the nearest
 * encodable rate (for example 128 -> 129 and 256 -> 255 at 48 kHz).
 */
LDACBT_API int ldacBT_set_bitrate( HANDLE_LDAC_BT hLdacBT, int bitrate )
{
    int64_t numerator;
    int frame_length;

    if( hLdacBT == NULL || bitrate <= 0 ){
        return LDACBT_E_FAIL;
    }
    if( hLdacBT->proc_mode != LDACBT_PROCMODE_ENCODE ){
        hLdacBT->error_code_api = LDACBT_ERR_HANDLE_NOT_INIT;
    ...
    numerator = (int64_t)bitrate * (1000 / 8) * hLdacBT->frm_samples;
    frame_length = (int)((numerator + hLdacBT->pcm.sf / 2)
                         / hLdacBT->pcm.sf) - LDACBT_FRMHDRBYTES;
    if( ldacBT_update_frmlen( hLdacBT, frame_length ) != LDACBT_S_OK ){
