...
void HaikuXdgToplevel::DoSendConfigure()
{
	if (Version() >= XDG_TOPLEVEL_CONFIGURE_BOUNDS_SINCE_VERSION) {
		BScreen scr(B_MAIN_SCREEN_ID);
		BRect scrFrame = scr.Frame();
		SendConfigureBounds(scrFrame.IntegerWidth() + 1, scrFrame.IntegerHeight() + 1);
	}
...
{
	if (app_id != NULL && (fWindow->Title() == NULL || strlen(fWindow->Title()) == 0))
		fWindow->SetTitle(app_id);
}
...
HaikuXdgToplevel *HaikuXdgToplevel::Create(HaikuXdgSurface *xdgSurface, uint32_t id)
{
	...
	xdgToplevel->fWindow = new WaylandWindow(xdgToplevel, BRect(), "", B_TITLED_WINDOW_LOOK, B_NORMAL_WINDOW_FEEL, 0);
...
{
	if (fZxdgDecoration != NULL) {
		fZxdgDecoration->fToplevel = NULL;
		fZxdgDecoration = NULL;
	}

	if (fXdgSurface != NULL) {
		fXdgSurface->fToplevel = NULL;
		if (fXdgSurface->Surface() != NULL)
