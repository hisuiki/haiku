...
HaikuServerDecoration *HaikuServerDecoration::Create(HaikuServerDecorationManager *manager, struct HaikuSurface *surface, uint32_t id)
{
	...
	serverDecor->fMode = OrgKdeKwinServerDecoration::modeServer;
	serverDecor->SendMode(OrgKdeKwinServerDecoration::modeServer);

	return serverDecor;
}

HaikuServerDecoration::~HaikuServerDecoration()
{
	if (fSurface != NULL) {
		fSurface->fServerDecoration = NULL;
		fSurface = NULL;
	}
}
...
{
	fMode = OrgKdeKwinServerDecoration::modeServer;
	SendMode(OrgKdeKwinServerDecoration::modeServer);
