...
{
	HaikuSeatGlobal *seat = Client() != NULL ? HaikuGetSeat(Client()) : NULL;
	if (seat != NULL) {
	...
	if (fXdgSurface != NULL) {
		fXdgSurface->fSurface = NULL;
		fXdgSurface = NULL;
	}

	if (fServerDecoration != NULL) {
		fServerDecoration->fSurface = NULL;
		fServerDecoration = NULL;
	}

	if (fSubsurface != NULL) {
		fSubsurface->fSurface = NULL;
		fSubsurface = NULL;
	}
	...
	while (!SurfaceList().IsEmpty()) {
		HaikuSubsurface *subsurface = SurfaceList().RemoveHead();
...
void HaikuSurface::Invalidate()
{
	...
	if (fSubsurface != NULL || fDirty.CountRects() == 0) {
